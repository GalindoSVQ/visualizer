...
import meshLinesJson from "../../../../../data/mesh_lines.json";
import meshJson from "../../../../../data/mesh.json";
...
export default class Grid3DLayer extends CompositeLayer<Grid3DLayerProps> {
  ...
  rebuildData(reportBoundingBox: boolean): void {
    ...
    p.then(([points]) => {
      ...
      const propertyValueRange = [0, 0.35];
  ...
  renderLayers(): [PrivateLayer?] {
    ...
    const layer = new PrivateLayer(
      this.getSubLayerProps({
        ...
        discretePropertyValueNames: this.props.discretePropertyValueNames,
